...
Full writing and coding ruleset: `.github/copilot-instructions.md`

Short version: no AI buzzwords (delve, leverage, seamless, robust, tapestry, synergy), no sycophancy, no resume verbs, no forward projections. Be specific — numbers, names, examples. Every paragraph needs one concrete anchor. Repeat the right word rather than cycling synonyms.

For the banned-word reference tables, structural patterns, sentence construction rules, and the Seven Failure Modes checklist, see `.github/copilot-instructions.md` or run `/llmstrip` on any file to clean it.

---

*Auto-generated content note: the ECC branch (`ecc-tools/babbled-wear-*`) contains generated Codex skills that incorrectly describe this repo as TypeScript/test. Disregard that branch. This file and `AGENTS.md` are the correct source of truth.*
